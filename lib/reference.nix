...
let
  ...
  referenceResolution =
    ...
    let
      ...
      # required only to publish `query`, so the id is read where the record carries one rather
      # than assumed — a refusal that threw while building its own message would replace a named
      ...
      # ★ THE REFUSAL THAT CLOSES THE FUSED PREDICATE'S THIRD DEFECT, and the only one of this
      # construct's refusals that fires at MATERIALIZATION rather than at construction. The
      # delegate reads `null` as "no binding here", so a well-formed node whose projection is null
      # would be read as an ABSENCE and the value would vanish with nothing saying so. An empty
      # answer is never a refusal in this library — and here the converse is what bites: a refusal
      # must never arrive wearing the shape of an empty answer.
      requireNonNull =
        node: value:
        if value == null then
          refuse "referenceResolution" "result '${a.name}': node ${nodeLabel node} is admitted by 'wellFormed' and its 'project' returned null; the query authority reads null as NO BINDING HERE, so a null projection would be indistinguishable from an absent one"
        else
          value;
    ...
    if !(builtins.isAttrs a.engine) || !(a.engine ? query) then
    ...
    else
      {
        ...
        compute = a.engine.query {
          dataFilter = n: if a.wellFormed n then requireNonNull n (a.project n) else null;
          inherit (a) localShadowsImport importShadowsParent transitiveImports;
        };
      };
in
{
  inherit referenceResolution required;
}
